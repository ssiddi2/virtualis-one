...
export class EpicAdapter implements EMRAdapter {
  ...
  async searchPatients(query: { name?: string; mrn?: string; dob?: string }): Promise<Patient[]> {
  ...
  async createOrder(order: Order): Promise<Order> {
    const result = await this.fhir.request<any>('/ServiceRequest', {
      ...
      body: JSON.stringify({
        ...
        priority: order.priority, code: { coding: [{ code: order.code, display: order.description }] },
  ...
  private mapPatient(f: any): Patient {
    const n = f.name?.[0] || {};
    return {
      id: f.id, mrn: f.identifier?.find((i: any) => i.type?.coding?.[0]?.code === 'MR')?.value || '',
      firstName: n.given?.join(' ') || '', lastName: n.family || '', dob: f.birthDate || '', gender: f.gender || 'unknown',
      phone: f.telecom?.find((t: any) => t.system === 'phone')?.value, email: f.telecom?.find((t: any) => t.system === 'email')?.value,
    };
  }
}
